...
import com.ordering.system1.repository.OrderRepository;
import com.ordering.system1.repository.ItemRepository;
import com.ordering.system1.repository.StaffRepository;
...
public class DashboardController {
    ...
    public Map<String, Object> getOrderStatus() {
        List<Order> orders = orderRepository.findAll();
        Map<String, Object> result = new HashMap<>();
        result.put("labels", List.of("Completed", "Pending", "Cancelled", "Confirmed"));
        result.put("data", List.of(
            orders.stream().filter(o -> o.getStatus().equals("COMPLETED")).count(),
            orders.stream().filter(o -> o.getStatus().equals("PENDING")).count(),
            orders.stream().filter(o -> o.getStatus().equals("CANCELLED")).count(),
            orders.stream().filter(o -> o.getStatus().equals("CONFIRMED")).count()
        ));
        return result;
    }
}